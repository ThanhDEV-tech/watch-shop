...
defineProps({
  ...
  markPaidError: { type: String, default: '' },
})

const emit = defineEmits(['close', 'mark-paid'])

const confirmMarkPaid = () => {
  if (!window.confirm('Chỉ dùng khi chắc chắn khách đã thanh toán thành công qua kênh khác. Thao tác này không thể hoàn tác.')) return
...
<template>
  <BaseModal max-width="lg" aria-labelledby="order-detail-title" @close="emit('close')">
      <section class="flex max-h-[90vh] w-full min-w-0 flex-col overflow-hidden rounded-xl border border-surface-variant bg-background shadow-2xl">
        ...
        <div v-if="loading" class="flex min-h-[320px] w-full min-w-0 items-center justify-center p-md text-body-sm text-on-surface-variant">
        ...
        <div v-else-if="order" class="w-full min-w-0 flex-1 overflow-y-auto p-md">
          ...
          <section class="mt-md w-full min-w-0">
            <h3 class="w-full font-display text-body-md font-semibold text-on-surface">Khóa học đã mua</h3>
            <div class="mt-sm w-full min-w-0 divide-y divide-surface-variant overflow-hidden rounded-lg border border-surface-variant bg-surface">
              <article v-for="item in order.items" :key="item.id" class="flex w-full min-w-0 items-center gap-sm p-sm">
                <img v-if="item.course?.thumbnail_url" :src="item.course.thumbnail_url" :alt="item.course.title" class="h-14 w-20 shrink-0 rounded object-cover" />
                ...
                <div class="w-full min-w-0"><p class="line-clamp-2 w-full text-body-sm font-medium text-on-surface">{{ item.course?.title ?? 'Khóa học không còn tồn tại' }}</p><p class="mt-xs w-full font-mono text-xs text-primary">{{ formatCurrency(item.price_snapshot ?? item.price) }}</p></div>
              </article>
              <p v-if="!order.items?.length" class="w-full p-md text-center text-body-sm text-on-surface-variant">Đơn hàng không có course item.</p>
            </div>
          </section>
        ...
        <footer v-if="!loading && !error" class="w-full min-w-0 border-t border-surface-variant bg-surface p-md">
          <p v-if="markPaidError" class="mb-sm w-full rounded-lg border border-error/40 bg-error/10 p-sm text-body-sm text-error">{{ markPaidError }}</p>
          <button
